...
def main():
    ...
    # Strict Comfy mode: hide manual file/project controls
    if config.get("comfy_strict_mode", False):
        for attr in ["load_btn", "save_btn", "save_as_btn", "load_project_btn"]:
    ...
    if project_data:
        ...
        try:
            ...
            if "source_video" in project and project["source_video"].get("path"):
                ...
                source_type = project["source_video"].get("type", "video")
            ...
            # Fall back to video_info (Comfy-driven path)
            elif "video_info" in project and isinstance(project["video_info"], dict) and project["video_info"].get("path"):
                source_path = project["video_info"]["path"]
                source_type = project["video_info"].get("type", "video")
                print(f"[ComfyUI Mask Editor] Loading video from video_info: {source_path} (type: {source_type})")
            else:
                source_path = None
                source_type = None
            ...
            if "mask_frames" in project:
                try:
                    restored_masks = 0
                    skipped_masks = 0
                    for frame_str, encoded_mask in project["mask_frames"].items():
                        ...
                        if mask is None:
                            skipped_masks += 1
                            continue
                        if not (hasattr(editor, 'mask_frames') and frame_idx < len(editor.mask_frames)):
                            skipped_masks += 1
                            continue

                        target_mask = editor.mask_frames[frame_idx]
                        th, tw = target_mask.shape[:2]
                        mh, mw = mask.shape[:2]
                        if (mh, mw) != (th, tw):
                            print(f"[ComfyUI Mask Editor] Mask size mismatch at frame {frame_idx}: incoming={(mh, mw)} target={(th, tw)}. Skipping this mask.")
                            skipped_masks += 1
                            continue

                        editor.mask_frames[frame_idx] = mask
                        restored_masks += 1

                    if restored_masks > 0 and hasattr(editor, 'mask_widget') and hasattr(editor, 'video_frames'):
                        current_idx = max(0, min(editor.current_frame_index, len(editor.mask_frames) - 1))
                        editor.mask_widget.set_mask(editor.mask_frames[current_idx], editor.video_frames[current_idx])
                        if hasattr(editor, 'update_mask_frame_tracking'):
                            original_mode = getattr(editor, 'drawing_mode', 'brush')
            ...
            if editor.source_type == "video":
                ...
                if cap.isOpened():
                    while True:
                        ...
                        frames.append(frame)
                    cap.release()
            elif editor.source_type == "image_sequence" and os.path.exists(editor.source_video_path):
                frames = load_image_sequence_optimized_mask_editor(editor.source_video_path)

            if frames:
                print(f"[ComfyUI Mask Editor] Reloaded {len(frames)} frames ({editor.source_type})")
                editor.current_frames = frames
                editor.reinitialize_with_frames(frames)
            else:
                print(f"[ComfyUI Mask Editor] Failed to reload source: {editor.source_video_path} (type: {editor.source_type})")
    ...
    if result == editor.Accepted:
        ...
        edited_masks = []
        if hasattr(editor, 'get_masks'):
            edited_masks = editor.get_masks()
        elif hasattr(editor, 'mask_frames'):
            edited_masks = editor.mask_frames

        if not edited_masks:
            raise RuntimeError("No masks available to export from mask editor")

        edited_masks_array = np.stack(edited_masks).astype(np.uint8)
        edited_masks_path = os.path.join(output_dir, "edited_masks.npy")
        np.save(edited_masks_path, edited_masks_array)
        print(f"[ComfyUI Mask Editor] Saved edited masks to: {edited_masks_path}")
        ...
        # Save raster masks for strict Comfy roundtrip support
        for idx, mask in enumerate(edited_masks_array):
            ok, encoded = cv2.imencode('.png', mask)
            if ok:
                project_data["mask_frames"][str(idx)] = base64.b64encode(encoded.tobytes()).decode('utf-8')
