...
class MaskTimelineWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Draw timeline track with extra top padding so frame text remains visible
        track_top = 24
        track_height = height - 29
        ...
        painter.drawText(5, 19, frame_text)
...
class InpaintingMaskEditor(QDialog):
    ...
    def __init__(self, video_frames, parent=None, initial_mode=None):
        ...
        # Load settings
        self.settings = QSettings('MaskEditor', 'VideoFrameProcessor')
        ...
        self._playback_session_id = 0  # Invalidate stale scheduled playback ticks on stop/pause
    ...
    def init_ui(self):
        ...
        mode_label_style = f"""
            QLabel {{
                ...
                padding: 3px 10px;
                border-radius: 999px;
                min-width: {max(68, int(round(78 * self._ui_scale)))}px;
                font-size: {mode_font_px}px;
                font-weight: bold;
                letter-spacing: 0.4px;
            }}
        """
        control_label_style = f"""
            QLabel {{
                color: #c9d1d9;
        ...
        value_chip_style = f"""
            QLabel {{
                ...
                border-radius: 999px;
                padding: 2px 8px;
                min-width: {value_chip_min_width}px;
                font-size: {compact_font_px}px;
                font-weight: 600;
            }}
        """
        ...
        brush_size_caption = QLabel("Size")
        brush_size_caption.setStyleSheet(control_label_style)
        toolbar_layout.addWidget(brush_size_caption)
        ...
        self.brush_size_label.setMinimumWidth(value_chip_min_width)
        self.brush_size_label.setAlignment(Qt.AlignCenter)
        self.brush_size_label.setStyleSheet(value_chip_style)
        ...
        self.lattice_size_label_text.setStyleSheet(control_label_style)
        ...
        self.lattice_size_label.setMinimumWidth(value_chip_min_width)
        self.lattice_size_label.setAlignment(Qt.AlignCenter)
        self.lattice_size_label.setStyleSheet(value_chip_style)
        ...
        def style_action_button(button, style_sheet):
            ...
            button.setMinimumWidth(max(wide_button_min_width, action_button_height * 2))
            button.setMinimumHeight(action_button_height)
            button.setStyleSheet(style_sheet)

        def style_meta_label(label):
            label.setStyleSheet(card_meta_label_style)

        self.clear_card, _, self.clear_section_label, clear_actions_row, clear_meta_row = create_action_card("Clear", "clear")
        ...
        style_action_button(self.clear_btn, clear_button_style)
        clear_actions_row.addWidget(self.clear_btn)
        ...
        style_action_button(self.clear_all_btn, clear_button_style)
        clear_actions_row.addWidget(self.clear_all_btn)
        clear_meta_row.addStretch()

        self.connect_card, _, self.connect_section_label, connect_actions_row, connect_meta_row = create_action_card("Connect", "connect")
        ...
        style_action_button(self.connect_masks_btn, connect_button_style)
        connect_actions_row.addWidget(self.connect_masks_btn)
        ...
        style_action_button(self.connect_masks_all_btn, connect_button_style)
        connect_actions_row.addWidget(self.connect_masks_all_btn)
        ...
        style_meta_label(self.connect_distance_label)
        connect_meta_row.addWidget(self.connect_distance_label)
        ...
        self.connect_distance_spinner.valueChanged.connect(
            lambda value: self.settings.setValue('connect_mask_distance_percent', int(value))
        )
        connect_meta_row.addWidget(self.connect_distance_spinner)
        connect_meta_row.addStretch()

        self.cleanup_card, _, self.cleanup_section_label, cleanup_actions_row, cleanup_meta_row = create_action_card("Keep Largest", "cleanup")
        ...
        style_action_button(self.keep_largest_mask_frame_btn, cleanup_button_style)
        cleanup_actions_row.addWidget(self.keep_largest_mask_frame_btn)
        ...
        style_action_button(self.keep_largest_masks_btn, cleanup_button_style)
        cleanup_actions_row.addWidget(self.keep_largest_masks_btn)
        cleanup_meta_row.addStretch()

        self.edge_card, _, self.edge_section_label, edge_actions_row, edge_meta_row = create_action_card("Snap To Edge", "edge")
        ...
        style_action_button(self.expand_edge_btn, edge_button_style)
        edge_actions_row.addWidget(self.expand_edge_btn)
        ...
        style_action_button(self.expand_edge_all_btn, edge_button_style)
        edge_actions_row.addWidget(self.expand_edge_all_btn)
        ...
        style_meta_label(self.edge_snap_label)
        edge_meta_row.addWidget(self.edge_snap_label)
        ...
        edge_meta_row.addWidget(self.edge_snap_spinner)
        edge_meta_row.addStretch()

        self.buffer_card, _, self.buffer_section_label, buffer_actions_row, buffer_meta_row = create_action_card("Buffer", "buffer")
        ...
        style_action_button(self.buffer_mask_btn, buffer_button_style)
        buffer_actions_row.addWidget(self.buffer_mask_btn)
        ...
        style_action_button(self.buffer_masks_all_btn, buffer_button_style)
        buffer_actions_row.addWidget(self.buffer_masks_all_btn)
        ...
        style_meta_label(self.buffer_percent_label)
        buffer_meta_row.addWidget(self.buffer_percent_label)
        ...
        buffer_meta_row.addWidget(self.buffer_percent_spinner)
        buffer_meta_row.addStretch()
        
        self.bake_card, _, self.bake_section_label, bake_actions_row, bake_meta_row = create_action_card("Liquify", "bake")
        ...
        style_action_button(self.bake_liquify_btn, edge_button_style)
        bake_actions_row.addWidget(self.bake_liquify_btn)
        bake_meta_hint = QLabel("Commit the current deformation")
        bake_meta_hint.setStyleSheet(card_meta_label_style)
        bake_meta_row.addWidget(bake_meta_hint)
        bake_meta_row.addStretch()
        self.bake_card.setVisible(False)
        ...
        self._action_card_columns = None
        self._action_card_layout_signature = None
        self.connect_card.setVisible(False)
        self.cleanup_card.setVisible(False)
        self.bake_card.setVisible(False)
        for card in self.action_cards:
            self.toolbar_actions_grid.addWidget(card)
        ...
        timeline_container.setContentsMargins(0, 8, 0, 0)
        ...
        try:
            self.play_pause_btn.pressed.connect(self.on_play_button_pressed, type=Qt.UniqueConnection)
            self.play_pause_btn.clicked.connect(self.on_play_button_clicked, type=Qt.UniqueConnection)
        except TypeError:
            self.play_pause_btn.pressed.connect(self.on_play_button_pressed)
            self.play_pause_btn.clicked.connect(self.on_play_button_clicked)
        self.play_pause_btn.setToolTip("Start or stop video playback")
        ...
        initial_fps = 24
        self.fps_spinner.setValue(initial_fps)
        self.playback_fps = initial_fps
        self.settings.setValue('playback_fps', initial_fps)
    ...
    def set_drawing_mode(self, mode):
        ...
        self.connect_card.setVisible(is_shape_mode)
    ...
    def on_vertex_count_changed(self, value):
        ...
        self.apply_vertex_count_setting(value, persist_setting=True)
    ...
    def apply_vertex_count_setting(self, value, persist_setting=True):
        """Apply vertex count to widget state and (optionally) persist settings."""
    ...
    def normalize_shape_keyframes_to_target_vertices(self, target_vertices):
        """Resample all keyframed shapes to a uniform vertex count."""
        ...
        for frame in self.mask_widget.shape_keyframes:
            for shape in self.mask_widget.shape_keyframes[frame]:
                if 'vertices' in shape and len(shape['vertices']) > 0:
                    if len(shape['vertices']) != target_vertices:
                        shape['vertices'] = self.mask_widget.resample_vertices(shape['vertices'], target_vertices)
                    shape['vertex_count'] = target_vertices
                    normalized_count += 1
    ...
    def _queue_navigation_carry_frames(self, target_frame):
        ...
        traversed_frames = [
            frame
            for frame in range(current_frame + step, target_frame + step, step)
            if 0 <= frame < len(self.mask_frames)
        ]
        if not traversed_frames:
            return []
        ...
        for frame in traversed_frames:
            if frame in pending_frames:
                continue
            pending_frames.add(frame)
            newly_added.append(frame)
        self._recent_brush_navigation_pending_frames = pending_frames
        return newly_added

    def _flush_pending_brush_navigation_mask(self):
        pending_frames = sorted(getattr(self, "_recent_brush_navigation_pending_frames", set()))
        if not pending_frames:
            self.clear_recent_brush_navigation_mask()
            return False

        changed = self._propagate_active_brush_mask(pending_frames)
        if changed:
            self.update_display()
        self.clear_recent_brush_navigation_mask()
        return changed
    ...
    def _propagate_active_brush_mask(self, frame_indices):
        ...
        affected_frames = valid_frames.copy()
        boundary_masks = {}
        if (
            self.drawing_mode in ["shape", "liquify"] and
            hasattr(self.mask_widget, "shape_keyframes") and
            any(frame not in self.mask_widget.shape_keyframes for frame in valid_frames)
        ):
            min_frame = min(valid_frames)
            max_frame = max(valid_frames)
            boundary_candidates = [min_frame - 1, max_frame + 1]
            boundary_masks = {
                frame: mask
                for frame, mask in (
                    (frame, self._render_effective_mask_for_frame(frame))
        ...
        if self.drawing_mode in ["shape", "liquify"] and hasattr(self.mask_widget, "shape_keyframes"):
            ...
            if hasattr(self.mask_widget, "_temp_interpolated_shapes"):
                if source_frame is not None:
                    self.mask_widget._temp_interpolated_shapes.pop(int(source_frame), None)
    ...
    def update_apply_button_state(self):
        ...
        if not hasattr(self, 'apply_current_btn'):
            return
    ...
    def clear_all_frames(self):
        ...
        confirm = QMessageBox.warning(
            self,
            "Clear All Frames",
            "This will permanently clear masks for every frame.\n\nDo you want to continue?",
        ...
        # Remove all persistent shape/keyframe state.
        if hasattr(self.mask_widget, 'shape_keyframes'):
            self.mask_widget.shape_keyframes.clear()
        if hasattr(self, '_original_shape_keyframes'):
            del self._original_shape_keyframes
        if hasattr(self.mask_widget, 'reset_sequence_reference_signature'):
            self.mask_widget.reset_sequence_reference_signature()
        if hasattr(self.mask_widget, 'invalidate_shape_cache'):
            self.mask_widget.invalidate_shape_cache()
        ...
        # Clear every temporary/preview state so no residue survives frame changes.
        if hasattr(self.mask_widget, '_temp_shapes'):
            self.mask_widget._temp_shapes = None
        if hasattr(self.mask_widget, '_temp_interpolated_shapes'):
            self.mask_widget._temp_interpolated_shapes = {}
        if hasattr(self.mask_widget, '_temp_deformed_shapes'):
            self.mask_widget._temp_deformed_shapes = None
        if hasattr(self.mask_widget, 'temp_shape_mask'):
            self.mask_widget.temp_shape_mask = None
        if hasattr(self.mask_widget, 'current_stroke_points'):
            self.mask_widget.current_stroke_points = []
        if hasattr(self.mask_widget, 'is_drawing'):
            self.mask_widget.is_drawing = False
        if hasattr(self.mask_widget, 'selected_vertex_index'):
            self.mask_widget.selected_vertex_index = None
        if hasattr(self.mask_widget, 'selected_shape_index'):
            self.mask_widget.selected_shape_index = None

        # Clear liquify working state too.
        if hasattr(self.mask_widget, 'liquify_deformation_field'):
            self.mask_widget.liquify_deformation_field = None
        if hasattr(self.mask_widget, 'liquify_original_shapes'):
            self.mask_widget.liquify_original_shapes = None
        if hasattr(self.mask_widget, 'is_liquifying'):
            self.mask_widget.is_liquifying = False

        # Clear the current visible mask immediately.
        ...
        # Restore focus to mask widget for keyboard shortcuts
        self.mask_widget.setFocus()
        QMessageBox.information(self, "Success", "All frame masks were cleared")

    def _find_nearest_vertex_pair(self, source_vertices, target_vertices):
        """Find the nearest pair of vertices between two shapes."""
        min_dist_sq = None
        nearest_pair = None
        for sx, sy in source_vertices:
            for tx, ty in target_vertices:
                dx = float(tx) - float(sx)
                dy = float(ty) - float(sy)
                dist_sq = dx * dx + dy * dy
                if min_dist_sq is None or dist_sq < min_dist_sq:
                    min_dist_sq = dist_sq
                    nearest_pair = ((float(sx), float(sy)), (float(tx), float(ty)))
        if nearest_pair is None:
            return None, float("inf")
        return nearest_pair, float(np.sqrt(min_dist_sq))

    def _build_rect_bridge(self, start_pt, end_pt, frame_diag):
        """Build a clean 4-vertex rectangular bridge with no intermediate vertices."""
        sx, sy = start_pt
        ex, ey = end_pt
        dx = ex - sx
        dy = ey - sy
        ...
        nx = -dy / distance
        ny = dx / distance
        bridge_half_width = max(4.0, min(frame_diag * 0.02, max(8.0, distance * 0.08)))

        p1 = [sx + nx * bridge_half_width, sy + ny * bridge_half_width]
        p2 = [sx - nx * bridge_half_width, sy - ny * bridge_half_width]
        p3 = [ex - nx * bridge_half_width, ey - ny * bridge_half_width]
        p4 = [ex + nx * bridge_half_width, ey + ny * bridge_half_width]
    ...
    def _connect_floating_masks_frame(self, frame_index):
        """Create bridge shapes for one frame. Returns (bridges, skipped_far)."""
        if not hasattr(self.mask_widget, 'shape_keyframes'):
            return [], 0

        shapes = self.mask_widget.shape_keyframes.get(frame_index)
        ...
        for idx, shape in enumerate(shapes):
            ...
            if area <= 0:
                continue
            shape_entries.append({'idx': idx, 'area': area, 'vertices': vertices})

        if len(shape_entries) < 2:
            return [], 0

        frame_h, frame_w = self.video_frames[frame_index].shape[:2]
        frame_diag = float(np.hypot(frame_w, frame_h))
        distance_percent = float(self.connect_distance_spinner.value()) if hasattr(self, 'connect_distance_spinner') else 20.0
        max_connection_distance = max(24.0, frame_diag * (distance_percent / 100.0))

        shape_entries.sort(key=lambda item: item['area'], reverse=True)

        bridges = []
        skipped_far = 0

        for small in shape_entries[1:]:
            nearest_pair = None
            nearest_distance = float("inf")

            # Prefer connecting each smaller mask to a larger/equal-area mask.
            for candidate in shape_entries:
                if candidate['idx'] == small['idx']:
                    continue
                if candidate['area'] < small['area']:
                    continue
                pair, dist = self._find_nearest_vertex_pair(small['vertices'], candidate['vertices'])
                if pair is not None and dist < nearest_distance:
                    nearest_pair = pair
                    nearest_distance = dist

            # Fallback: if no larger mask exists, connect to nearest other mask.
            if nearest_pair is None:
                for candidate in shape_entries:
                    if candidate['idx'] == small['idx']:
                        continue
            ...
            if nearest_pair is None:
                continue
            if nearest_distance > max_connection_distance:
                skipped_far += 1
                continue

            bridge_shape = self._build_rect_bridge(nearest_pair[0], nearest_pair[1], frame_diag)
            if bridge_shape is not None:
                bridges.append(bridge_shape)

        return bridges, skipped_far

    def _merge_connected_shapes_for_frame(self, frame_index):
        """Boolean-merge all shapes on a frame into contour-derived polygons."""
        if not hasattr(self.mask_widget, 'shape_keyframes'):
            return 0
        ...
        frame_h, frame_w = self.video_frames[frame_index].shape[:2]
        merged_mask = np.zeros((frame_h, frame_w), dtype=np.uint8)
        ...
        self.mask_widget.shape_keyframes[frame_index] = merged_shapes
        return len(merged_shapes)

    def connect_floating_masks_current_frame(self):
        """Connect smaller floating masks to larger nearby masks on current frame."""
        if self.drawing_mode not in ["shape", "liquify"]:
            QMessageBox.information(self, "Shape Mode Required", "Switch to Shape or Liquify mode to connect floating masks.")
            return

        if not hasattr(self.mask_widget, 'shape_keyframes'):
            QMessageBox.warning(self, "No Shapes", "No shape keyframe data is available.")
            return

        bridges, skipped_far = self._connect_floating_masks_frame(self.current_frame_index)
        if not bridges:
        ...
        QMessageBox.information(
            self,
            "Connected",
            f"Added {len(bridges)} mask connector(s) on this frame.\n"
            f"Merged result: {merged_shape_count} shape(s).\n"
            f"Skipped {skipped_far} shape(s) beyond distance limit."
        )

    def connect_floating_masks_all_frames(self):
        """Connect floating masks across all frames that already have shape masks."""
        if self.drawing_mode not in ["shape", "liquify"]:
            QMessageBox.information(self, "Shape Mode Required", "Switch to Shape or Liquify mode to connect floating masks.")
            return

        if not hasattr(self.mask_widget, 'shape_keyframes') or not self.mask_widget.shape_keyframes:
            QMessageBox.information(self, "No Shapes", "No shape keyframes are available.")
            return

        frame_indices = sorted(
            frame for frame, shapes in self.mask_widget.shape_keyframes.items()
        ...
        total_skipped_far = 0
        touched_frames = 0
        total_merged_shapes = 0
        for frame_idx in frame_indices:
            bridges, skipped_far = self._connect_floating_masks_frame(frame_idx)
            total_skipped_far += skipped_far
            if bridges:
                self.mask_widget.shape_keyframes[frame_idx].extend(bridges)
                total_merged_shapes += self._merge_connected_shapes_for_frame(frame_idx)
                total_bridges += len(bridges)
                touched_frames += 1

        if total_bridges == 0:
            QMessageBox.information(self, "No Bridges Added", "No valid floating-mask connections were created on eligible frames.")
            return
        ...
        QMessageBox.information(
            self,
            "Connected",
            f"Added {total_bridges} connector(s) across {touched_frames} frame(s).\n"
            f"Merged result: {total_merged_shapes} total shape(s).\n"
            f"Skipped {total_skipped_far} shape(s) beyond distance limit."
        )
    ...
    def save_project(self, filepath=None):
        ...
        if filepath is None or filepath == '' or not isinstance(filepath, str):
            ...
            settings = QSettings('MaskEditor', 'MaskEditor')
    ...
    def _relayout_action_cards(self, columns):
        if not hasattr(self, "toolbar_actions_grid") or not hasattr(self, "action_cards"):
            return

        columns = max(1, int(columns))
        visible_cards = [card for card in self.action_cards if card.isVisible()]
        layout_signature = tuple(id(card) for card in visible_cards)
        if (
            getattr(self, "_action_card_columns", None) == columns and
            getattr(self, "_action_card_layout_signature", None) == layout_signature
        ):
            return

        while self.toolbar_actions_grid.count():
            item = self.toolbar_actions_grid.takeAt(0)
            widget = item.widget()
            if widget is not None:
                widget.setParent(self.toolbar_actions_grid_host)

        for index, card in enumerate(visible_cards):
            row = index // columns
            column = index % columns
            self.toolbar_actions_grid.addWidget(card, row, column)

        for column in range(columns):
            self.toolbar_actions_grid.setColumnStretch(column, 1)

        self._action_card_columns = columns
        self._action_card_layout_signature = layout_signature
        self.toolbar_actions_grid_host.updateGeometry()
    ...
    def force_pause_playback(self):
        """Hard pause handler used by UI pause button."""
        self.pause_playback()
    
    def force_stop_playback(self):
        """Hard stop handler used by UI stop button."""
        self.force_pause_playback()
        if hasattr(self, "playback_start_frame") and self.playback_start_frame != self.current_frame_index:
            try:
                ...
                print(f"Error during force stop playback frame change: {e}")
    ...
    def on_play_button_pressed(self):
        """Mouse-down handler: stop immediately while playing."""
        if self.is_playing:
            self._ignore_next_play_click = True
            self.stop_playback()

    def on_play_button_clicked(self):
        """Mouse-up click handler: only starts playback when currently stopped."""
        if self._ignore_next_play_click:
            self._ignore_next_play_click = False
            return
        if not self.is_playing:
            self.start_playback()

    def _schedule_next_playback_tick(self, session_id):
        """Schedule next playback step in a way that can be invalidated by session id."""
        interval = max(1, int(1000 / self.playback_fps))
        QTimer.singleShot(interval, lambda sid=session_id: self.advance_frame(sid))
    ...
    def start_playback(self):
        ...
        self.play_pause_btn.setText("Stop")
        self.playback_start_frame = self.current_frame_index
        self._playback_session_id += 1

        if self.playback_timer.isActive():
            self.playback_timer.stop()
        self._schedule_next_playback_tick(self._playback_session_id)
    ...
    def stop_playback(self):
        """Stop playback and keep current frame (resume from here on next play)."""
        self.is_playing = False
        self._playback_session_id += 1
        self.play_pause_btn.setText("Play")
        if self.playback_timer.isActive():
    ...
    def advance_frame(self, session_id=None):
        """Advance to next frame during playback"""
        if session_id is not None and session_id != self._playback_session_id:
            return
        if not self.is_playing:
            return
        ...
        # Schedule the next playback tick explicitly; stale ticks are ignored via session id.
        if self.is_playing:
            self._schedule_next_playback_tick(self._playback_session_id)
    ...
    def keep_largest_mask_all_keyframes(self):
        """For each shape keyframe, keep only the largest mask by contour area."""
        if self.drawing_mode not in ["shape", "liquify"]:
            QMessageBox.information(self, "Shape Mode Required", "Switch to Shape or Liquify mode to prune to the largest mask.")
            return
        ...
        for frame_idx, shapes in list(self.mask_widget.shape_keyframes.items()):
            ...
            for shape in shapes:
                ...
                if area <= 0:
                    continue
                valid_shapes.append((shape, area))
            ...
            self.mask_widget.shape_keyframes[frame_idx] = [largest_shape]
            removed = max(0, original_count - 1)
            if removed > 0:
                frames_modified += 1
                removed_shapes += removed

        if frames_modified == 0:
            QMessageBox.information(self, "No Changes", "No keyframes had smaller masks to remove.")
            return
        ...
        QMessageBox.information(
            self,
            "Largest Mask Kept",
            f"Updated {frames_modified} keyframe(s).\nRemoved {removed_shapes} smaller mask(s)."
        )

    def keep_largest_mask_current_frame(self):
        """Keep only the largest mask on the current keyframe."""
        if self.drawing_mode not in ["shape", "liquify"]:
            QMessageBox.information(self, "Shape Mode Required", "Switch to Shape or Liquify mode to prune to the largest mask.")
            return

        if not hasattr(self.mask_widget, 'shape_keyframes'):
            QMessageBox.information(self, "No Shapes", "No shape keyframe data is available.")
            return

        shapes = self.mask_widget.shape_keyframes.get(self.current_frame_index, [])
        if not shapes or len(shapes) <= 1:
            QMessageBox.information(self, "No Changes", "Current frame does not have multiple masks to clean up.")
            return
        ...
        self.update_display()
        self.mask_widget.setFocus()
        QMessageBox.information(self, "Largest Mask Kept", f"Removed {removed_count} smaller mask(s) on current frame.")
...
class MaskDrawingWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.target_vertex_count = 150  # Default vertex count for shapes
    ...
    def set_mask(self, mask, video_frame):
        ...
        if video_frame is not None:
            frame_shape = (int(video_frame.shape[0]), int(video_frame.shape[1]))
        
        if frame_shape != self._last_frame_shape:
    ...
    def fit_to_view(self):
        """Scale image so the full frame is visible in the widget."""
        if self.video_frame is None:
            return
        img_h, img_w = self.video_frame.shape[:2]
    ...
    def interpolate_shapes_spline(self, frame):
        ...
        if not bool(getattr(self.parent_editor, 'enable_smooth_interpolation', True)):
            return None
    ...
    def compute_signature_distance(self, sig1, sig2):
        ...
        try:
            ...
            angle_dist = abs(angle1 - angle2) / np.pi  # Normalize by Ï€
        ...
        try:
            ...
            if frame not in self.shape_keyframes:
                ...
                if current_shapes:
                    ...
                    target_vertex_count = 150  # Default
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            if (hasattr(self, '_temp_interpolated_shapes') and 
                ...
                warning_text = "Temporary Shape - Switch frames to confirm save/discard"
    ...
    def should_use_smooth_shapes(self):
        ...
        return bool(getattr(self.parent_editor, 'enable_smooth_shapes', True))
    ...
    def draw_shortcuts_overlay(self, painter):
        ...
        if self.drawing_mode in ["shape", "liquify"]:
            shortcuts.append(("Alt+Left/Right", "Prev/Next keyframe"))
        else:
        ...
        rows_per_col = len(shortcuts)
        columns = 1
        while columns < 3:
            rows_per_col = (len(shortcuts) + columns - 1) // columns
            needed_height = padding * 2 + header_height + (rows_per_col * row_height)
            if needed_height <= available_height:
                break
            columns += 1

        content_width = overlay_width - (padding * 2) - ((columns - 1) * column_gap)
        col_width = max(150, content_width // columns)
        key_col_width = min(140, max(100, int(col_width * 0.42)))
        desc_col_width = max(40, col_width - key_col_width - 10)
        overlay_height = min(available_height, padding * 2 + header_height + (rows_per_col * row_height))
        ...
        for idx, (key, description) in enumerate(shortcuts):
            ...
            col_x = x + padding + (col * (col_width + column_gap))
            row_y = row_start_y + (row * row_height)

            key_rect = QRect(col_x, row_y, key_col_width, row_height)
            desc_rect = QRect(col_x + key_col_width + 10, row_y, desc_col_width, row_height)

            key_text = metrics.elidedText(key, Qt.ElideRight, key_rect.width())
            desc_text = metrics.elidedText(description, Qt.ElideRight, desc_rect.width())

            painter.setPen(QPen(QColor(100, 200, 255), 1))
            painter.drawText(key_rect, Qt.AlignLeft | Qt.AlignVCenter, key_text)
            painter.setPen(QPen(QColor(220, 220, 220), 1))
            painter.drawText(desc_rect, Qt.AlignLeft | Qt.AlignVCenter, desc_text)
