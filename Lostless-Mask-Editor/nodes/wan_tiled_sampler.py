...
from ..tiled_sampler_schedulers import (
...
from ..tiled_sampler_utils import get_sigmas, sigmas_to_timesteps, apply_lora
from ..context import get_context_scheduler

# Simple rearrange implementation for our needs
def rearrange(tensor, pattern, **axes_lengths):
    """Simple rearrange for 'b c f h w -> b (f h w) c' pattern"""
    if pattern == "b c f h w -> b (f h w) c":
        b, c, f, h, w = tensor.shape
        return tensor.permute(0, 2, 3, 4, 1).reshape(b, f * h * w, c)
    else:
        raise NotImplementedError(f"Rearrange pattern '{pattern}' not implemented")

# WANTiledSampler - Tiled video generation for memory-efficient processing
# Adapted from ComfyUI-WanVideoWrapper for WanVace-pipeline
# Processes large videos by breaking them into tiles with overlap blending
...
class WindowTracker:
    ...
    def get_window_id(self, frames_tuple):
        # Convert tuple to a hashable string key
        key = str(frames_tuple)
        if key in self.windows:
            return self.windows[key]
        else:
            window_id = len(self.windows)
            self.windows[key] = window_id
            self.cache_states[window_id] = [None, None]  # Init cache state
            return window_id

    def get_teacache(self, window_id, base_state):
        # This seems to be a redundant implementation detail from the original sampler
        # The main logic uses self.cache_states, so we align with that.
        return self.cache_states.get(window_id, [None, None])
...
def tiled_predict_with_cfg(
    ...
    **kwargs,
    ...
    # Use CPU for accumulation to save VRAM
    output = torch.zeros_like(z, device='cpu')
    count = torch.zeros_like(z, device='cpu')

    # Create blend mask function for memory efficiency
    def create_blend_mask(h, w, padding, device='cpu', dtype=torch.float32):
    ...
    for y in range(0, height, stride_y):
        for x in range(0, width, stride_x):
            ...
            # Create blend mask for actual tile size
            tile_h, tile_w = pred_tile.shape[3], pred_tile.shape[4]
            mask_key = (tile_h, tile_w)
            if mask_key not in blend_mask_cache:
                blend_mask_cache[mask_key] = create_blend_mask(
                    tile_h, tile_w, tile_padding, device='cpu', dtype=z.dtype
                )
            effective_blend_mask = blend_mask_cache[mask_key]
            
            # Move to CPU before accumulating to save VRAM
            tile_result = (pred_tile * effective_blend_mask.to(pred_tile.device)).cpu()
    ...
    return (output / torch.clamp(count, min=1e-6)).to(z.device)
...
class WANTiledSampler:
    ...
    def process(
        ...
        if image_cond is not None:
            ...
            if transformer_options is not None:
                ...
                if ATI_tracks is not None:
                    # ATI motion patch not available in standalone version
                    log.warning("ATI motion patch not available in standalone WANTiledSampler")
                    patch_motion = None
        ...
        if context_options is not None:
            ...
            # get_context_scheduler already imported at top
        ...
        # Latent preview not available in standalone version
        callback = None
        if hasattr(args, 'preview_method') and args.preview_method in [
            ...
            try:
                ...
                log.warning("Latent preview not available in standalone WANTiledSampler")
                callback = None
                ...
                if tiling_enabled:
                    if batched_cfg:
                        raise NotImplementedError(
                            "Tiling is not supported with batched_cfg."
                        )
                    if phantom_latents is not None:
                        raise NotImplementedError(
                            "Tiling is not supported with Phantom Latents."
                        )
                    if controlnet is not None:
                        raise NotImplementedError(
                            "Tiling is not supported with ControlNet."
                        )
                    if recammaster is not None:
                        raise NotImplementedError(
                            "Tiling is not supported with RecamMaster."
                        )
                    ...
                    blend_mask = torch.ones(
                        (tile_height, tile_width), device=z.device, dtype=z.dtype
                    )
                    if tile_padding > 0:
                        feather = tile_padding
                        for i in range(feather):
                            value = (i + 1) / feather
                            if i < tile_height:
                                blend_mask[i, :] *= value
                            ...
                            if i < tile_width:
                                blend_mask[:, i] *= value
                                blend_mask[:, -i - 1] *= value
                    blend_mask = blend_mask.unsqueeze(0).unsqueeze(
                        0
        ...
        try:
            # print_memory function may not be available
            if 'print_memory' in globals():
                print_memory(device)
            torch.cuda.reset_peak_memory_stats(device)
        except:
            pass
