...
app.registerExtension({
    ...
    async setup() {
        ...
        app.graphToPrompt = async function() {
            ...
            for (const node of app.graph._nodes) {
                ...
                if ((node.type === "MaskEditor" || node.comfyClass === "MaskEditor") && node.maskData) {
                    console.log(`[Mask Editor] Found mask editor node ${node.id} with maskData`);
                    const maskDataWidget = node.widgets.find(w => w.name === "mask_data");
                    if (maskDataWidget) {
                        console.log(`[Mask Editor] Found mask_data widget`);
                        ...
                        console.log(`[Mask Editor] Updated mask editor node ${node.id} with mask data`);
                        console.log(`[Mask Editor] Data keys:`, Object.keys(node.maskData));
                        console.log(`[Mask Editor] Widget value set to:`, jsonData.substring(0, 200) + "...");
                    }
                }
            }
    ...
    async nodeCreated(node) {
        console.log("[Mask Editor] Node created:", node.comfyClass);
        if (node.comfyClass === "MaskEditor") {
            const hasLegacyMaskWidget = Array.isArray(node.widgets) && node.widgets.some(w => w?.name === "mask_data");
            if (!hasLegacyMaskWidget) {
                console.log("[Mask Editor] Skipping legacy UI hooks for Comfy-driven mask editor node");
                return;
            }
            console.log("[Mask Editor] Adding widgets to mask editor node");
            // Add button widget
            const buttonWidget = node.addWidget(
                ...
                "open",
                () => {
                    console.log("[Mask Editor] Button clicked, launching editor...");
                    ...
                    if (node.maskData && node.maskData.output_dir) {
                        ...
                        console.log("[Mask Editor] Sending existing output_dir:", requestData.output_dir);
                    }
                    
                    // Call Python to launch the mask editor
                    api.fetchApi("/mask_editor/launch", {
                        method: "POST",
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            console.log("[Mask Editor] Mask editor closed, data:", data);
                            console.log("[Mask Editor] mask_data structure:", data.mask_data);
                            ...
                            if (data.mask_data) {
                                ...
                                if (maskDataWidget) {
                                    ...
                                    console.log("[Mask Editor] Set widget value to:", maskDataWidget.value.substring(0, 200) + "...");
                                } else {
                                    console.log("[Mask Editor] ERROR: Could not find mask_data widget!");
                                }
                                if (statusWidget) {
